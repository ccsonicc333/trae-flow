...
private let logger = Logger(subsystem: "ai.traeflow.app", category: "Hooks")
...
private extension BridgeEnvelope {
    /// Shared encoder used to render a stable, human-readable JSON snapshot
    /// of the envelope for `SessionState.lastEnvelopeJSON`.
    private static let prettyEnvelopeEncoder: JSONEncoder = {
    ...
    /// Pretty-printed JSON representation of the envelope, or `nil` if encoding fails.
    var prettyEnvelopeJSON: String? {
        guard let data = try? Self.prettyEnvelopeEncoder.encode(self),
              let json = String(data: data, encoding: .utf8) else {
            return nil
        }
        return json
    ...
    var hookEvent: HookEvent {
        ...
        return HookEvent(
            ...
            cwd: resolvedCWD,
            event: eventType,
            status: Self.mapStatus(eventType: eventType, status: status?.kind, notificationType: metadata["notification_type"], provider: provider, clientKind: metadata["client_kind"]),
            ...
            lastEnvelopeJSON: prettyEnvelopeJSON
        )
    }
    ...
    private static func mapStatus(
        ...
        case "SessionStart", "SubagentStop":
            return "waiting_for_input"
        case "Stop":
            // Trae IDE hooks 把 Stop 作为会话终止生命周期事件，
            // 需映射为 ended 让会话在列表中保留为已完成状态。
            if clientKind?.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() == "trae" {
                return "ended"
            }
            return "idle"
    ...
    private static func makeClientInfo(
        ...
        let explicitClientBundleIdentifier = explicitBundleID?
            ...
            .lowercased()
        let effectiveExplicitKind = explicitKind
        let effectiveExplicitName = explicitName
        ...
        // 与 TRAEFLOW 对齐：runtimeProfile 为单个 "trae" profile，覆盖所有 TRAE 变体。
        // 变体区分（Trae / Trae CN / SOLO / Work）由终端 bundle ID 经
        // TraeVariant.fromBundleIdentifier 解析，保留到 clientInfo.bundleIdentifier。
        let resolvedProfile = matchedProfile

        // 当 bridge 未显式传递 bundle ID 时，从终端 bundle ID 反查 TRAE 变体，
        // 保证 Trae CN / SOLO 等变体的 bundle ID 不丢失。
        let terminalBundleVariant = terminalBundleID
            .flatMap { TraeVariant.fromBundleIdentifier($0) }
        let resolvedBundleID = explicitBundleID
            ?? terminalBundleVariant?.bundleIdentifier
            ?? resolvedProfile?.bundleIdentifiers.first
...
private extension BridgeProvider {
    var sessionProvider: SessionProvider { .trae }
}
...
class HookSocketServer {
    ...
    static let healthCheckRequest = #"{"type":"trae-flow-health-check"}"#
    ...
    private let queue = DispatchQueue(label: "ai.traeflow.app.socket", qos: .userInitiated)
