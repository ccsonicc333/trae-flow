...
//  TraeFlow
//
//  Finds tmux targets for TRAE processes
//

import Foundation

/// Finds tmux session/window/pane targets for TRAE processes
actor TmuxTargetFinder {
    static let shared = TmuxTargetFinder()

    private init() {}

    /// Find the tmux target for a given TRAE session PID
    func findTarget(forSessionPid sessionPid: Int) async -> TmuxTarget? {
    ...
    func isSessionPaneActive(sessionPid: Int) async -> Bool {
        ...
        // Find which pane the TRAE session is in
