...
enum DiagnosticsLogRedactor {
    ...
    nonisolated static func sanitizedTraeHookDebugLine(_ line: String) -> String? {
...
actor DiagnosticsExporter {
    ...
    private static let maxTraeDebugJSONLLines = 120
    ...
    func exportArchive(to destinationURL: URL) async throws -> DiagnosticsExportResult {
        ...
        let tempRoot = fileManager.temporaryDirectory
            .appendingPathComponent("TraeFlow-Diagnostics-\(UUID().uuidString)", isDirectory: true)
        let exportRoot = tempRoot.appendingPathComponent("TraeFlow-Diagnostics-\(timestamp)", isDirectory: true)
        var warnings: [String] = []
        ...
        do {
            try copySanitizedTraeHookDebugLogs(
                from: preferredTraeHookDebugDirectory(),
    ...
    private func copySanitizedTraeHookDebugLogs(from sourceURL: URL, toRelativeDirectory relativePath: String, under rootURL: URL) throws {
        ...
        for file in files {
            ...
            if file.pathExtension.lowercased() == "jsonl" {
                try writeSanitizedTraeHookJSONL(from: file, to: destinationURL)
            } else {
        ...
        if !files.isEmpty {
            ...
            Export is limited to the \(Self.maxDebugFilesPerDirectory) most recent files and the last \(Self.maxTraeDebugJSONLLines) JSONL records per file.
            """
    ...
    private func writeSanitizedTraeHookJSONL(from sourceURL: URL, to destinationURL: URL) throws {
        let lines = try tailLines(
            from: sourceURL,
            maxLineCount: Self.maxTraeDebugJSONLLines,
            maxBytes: Self.maxDebugFileBytes
        )
        let sanitizedLines = lines.compactMap(DiagnosticsLogRedactor.sanitizedTraeHookDebugLine)
    ...
    private func writeRedactedTailCopy(from sourceURL: URL, to destinationURL: URL) throws {
        ...
        var output = result.wasTruncated
            ? "[TRAE FLOW diagnostics: file truncated to last \(Self.maxDebugFileBytes) bytes]\n"
            : ""
    ...
    private func copyRecentCrashReports(toRelativeDirectory relativePath: String, under rootURL: URL) throws {
        ...
        .filter { url in
            ...
            return name.hasPrefix("ping island") || name.hasPrefix("traeflow")
        }
        .sorted { lhs, rhs in
            let leftDate = (try? lhs.resourceValues(forKeys: [.contentModificationDateKey]).contentModificationDate) ?? .distantPast
            let rightDate = (try? rhs.resourceValues(forKeys: [.contentModificationDateKey]).contentModificationDate) ?? .distantPast
        ...
        let metadata: [String: Any] = await MainActor.run {
            [
                ...
                "settings": [
                    ...
                    "closedNotchTrailingContentMode": AppSettings.shared.closedNotchTrailingContentMode.rawValue,
    ...
    private func writeUnifiedLogs(to destinationURL: URL) async throws {
        let predicate = "subsystem == \"ai.traeflow.app\""
        try await writeCommandOutput(
    ...
    private func preferredTraeHookDebugDirectory() -> URL {
        fileManager.homeDirectoryForCurrentUser.appendingPathComponent(".trae-flow-debug/claude-hooks", isDirectory: true)
    }
