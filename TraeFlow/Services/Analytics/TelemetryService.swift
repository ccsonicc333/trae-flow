...
struct TelemetryConfiguration: Equatable, Sendable {
    ...
    nonisolated init(
        slsHost: String,
        project: String = "trae-flow",
        logstore: String = "trae-flow",
        topic: String = "product-telemetry",
        source: String = "trae-flow-macos",
    ...
    nonisolated init(infoDictionary: [String: Any] = Bundle.main.infoDictionary ?? [:]) {
        ...
        self.init(
            ...
            project: infoDictionary["PINGTelemetrySLSProject"] as? String ?? "trae-flow",
            logstore: infoDictionary["PINGTelemetrySLSLogstore"] as? String ?? "trae-flow",
            topic: infoDictionary["PINGTelemetrySLSTopic"] as? String ?? "product-telemetry",
            source: infoDictionary["PINGTelemetrySLSSource"] as? String ?? "trae-flow-macos",
...
actor TelemetryService {
    ...
    private static let logger = Logger(subsystem: "ai.traeflow.app", category: "Telemetry")
