        ...
        HStack(alignment: .center, spacing: density.rowSpacing) {
            ...
            VStack(alignment: .leading, spacing: density.rowDetailSpacing) {
                HStack(alignment: .firstTextBaseline, spacing: density.badgeSpacing) {
                    ...
                    Text(usesTitleOnlySubagentPresentation ? "" : session.displayTitle)
...
private struct HoverTerminalRoutedPromptNotice: View {
    ...
    var body: some View {
        Text(verbatim: AppLocalization.format(
            "已保留在%@中处理。TRAE FLOW 只提醒，不接管此处响应。",
            ...
            HStack(alignment: .center, spacing: HoverSessionLayout.headerSpacing) {
                ...
                VStack(alignment: .leading, spacing: 5) {
                    HStack(alignment: .firstTextBaseline, spacing: 8) {
                        ...
                        Text(usesCodexSubagentTitleOnlyPresentation ? "" : session.displayTitle)
...
private struct HoverSessionPreviewLines: View {
    ...
    private var lines: [HoverPreviewLine] {
        if session.shouldUseMinimalCompactPresentation || false {
            return []
        }
...
private struct HoverProviderGlyph: View {
    ...
    private var attentionIndicatorTone: NotchIndicatorTone? {
    ...
    var body: some View {
        ZStack(alignment: .topTrailing) {
            ...
            .background(attentionIndicatorTone == nil ? Color.white.opacity(0.04) : Color.clear)
            .clipShape(RoundedRectangle(cornerRadius: 8, style: .continuous))

            if let attentionIndicatorTone {
                PermissionIndicatorIcon(size: 11, color: attentionIndicatorTone.emphasisColor)
                    .offset(x: 2, y: -2)
            }
        }
    }
...
private enum HoverPreviewStyle {
    ...
    static func providerBadgeFill(for session: SessionState) -> Color {
        providerColor(for: session).opacity(session.provider == .trae ? 0.26 : 0.22)
    }
