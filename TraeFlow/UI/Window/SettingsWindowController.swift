    ...
    private init() {
        ...
        hostingController.rootView = SettingsWindowView(
            ...
            onMinimize: { [weak self] in
                ...
                window.orderOut(nil)
                self.publishVisibilityDidChange(isVisible: false)
            }
        )
    }
...
final class PresentationModeWelcomeWindowController: NSWindowController, NSWindowDelegate {
    ...
    func present(onComplete: @escaping (IslandSurfaceMode) -> Void) {
        ...
        // 首次启动的形态选择引导默认选中 Flow Island（刘海屏），避免新用户首次打开就看到独立悬浮宠物。
        let initialMode: IslandSurfaceMode = AppSettings.presentationModeOnboardingPending
            ? .notch
            : AppSettings.surfaceMode
        hostingController.rootView = AnyView(
            AppLocalizedRootView {
                PresentationModeWelcomeView(initialMode: initialMode) { [weak self] mode in
...
private struct HookInstallWelcomeView: View {
    ...
    private var subtitle: String {
        "TRAE FLOW 通过 Hooks 监听会话事件、显示通知与审批。可以一键安装默认配置，或选择仅启用部分事件。"
    }
        ...
        ZStack {
            ...
            VStack(alignment: .leading, spacing: 22) {
                VStack(alignment: .leading, spacing: 10) {
                    ...
                    Text(appLocalized: "你可以把 TRAE FLOW 放在屏幕顶部，也可以让宠物默认贴近当前激活窗口右下角显示。之后都能在设置里随时切换。")
