...
- If you change the Mineradio Bridge compat layer (Spec: `mineradio-bridge-compat-layer`), trace through `MineradioBridgeEngine` (JSContext + polyfills + native callbacks), `MineradioBridgeCoordinator` (WKWebView↔JSC message routing + binary proxy + login state), `MineradioBridgeUserScript` (content-script injection + `__mineradioDeliverJSON`), `MusicPlatform` (three-platform login model), `CustomAreaWebView` (`.mineradio` `ContentSource` + Bridge script injection + `decidePolicyFor` cross-host allow + message handler routing), `MineradioLoginView` (platform login WKWebView + cookie poll), `MineradioCompactView`, `LeftFeatureContainerView` (`.mineradio` expanded branch), `NotchView` (compact dispatch), `SettingsWindowView` (login status buttons + URL edit + logout), `LeftFeatureStore` (`ensureBuiltinMineradioFeature` + `updateMineradioPageURL`), and `TraeFlow/Resources/MineradioBridge/bridge-bundle.js` + `bridge-polyfills.js` (regenerate via esbuild from `Mineradio-Bridge-1.3.1/` if extension upgrades) so Bridge protocol, cookie sharing, login flow, and UI dispatch stay aligned.
