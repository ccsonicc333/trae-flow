...
final class NotchViewModelTests: XCTestCase {
    ...
    func testDetachedPresentationKeepsDockedFlowIslandVisible() async {
        await MainActor.run {
            let viewModel = makeViewModel()
            ...
            XCTAssertEqual(viewModel.closedPresentationOffsetY, 0)
            // 分离态不腐蚀 docked 状态：contentType/status 保持分离前的值
            XCTAssertEqual(viewModel.contentType, .instances)
            XCTAssertEqual(viewModel.status, .opened)
            // 分离态使用独立的 detachedContentType
            XCTAssertEqual(viewModel.detachedContentType, .instances)

            viewModel.redockAfterDetached()
            XCTAssertEqual(viewModel.presentationMode, .docked)
            // redock 不重置 docked 内容状态，但需把展开态收合，避免拖回后透明窗口占据屏幕上半区域
            XCTAssertEqual(viewModel.status, .closed)
            XCTAssertEqual(viewModel.contentType, .instances)
            XCTAssertFalse(viewModel.shouldHideWindowPresentation)
        }
