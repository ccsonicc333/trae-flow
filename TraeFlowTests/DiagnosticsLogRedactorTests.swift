...
final class DiagnosticsLogRedactorTests: XCTestCase {
    func testTraeHookJSONLRedactionOmitsInternalPayloads() throws {
        ...
        let lineObject: [String: Any] = [
            ...
            "arguments": ["TraeFlowBridge", "--source", "claude", "--socket-path", "/tmp/private.sock"],
            "environment": [
        ...
        let sanitizedLine = try XCTUnwrap(DiagnosticsLogRedactor.sanitizedTraeHookDebugLine(line))
    ...
    func testTraeHookJSONLRedactionHandlesInvalidJSONWithoutLeakingRawLine() throws {
        let rawLine = "{\"stdinRaw\":\"secret prompt\""

        let sanitizedLine = try XCTUnwrap(DiagnosticsLogRedactor.sanitizedTraeHookDebugLine(rawLine))
