...
final class DetachedIslandWindowControllerTests: XCTestCase {
    private var originalFloatingPetCustomScale: CGFloat = 0

    override func setUp() {
        super.setUp()
        originalFloatingPetCustomScale = AppSettings.floatingPetCustomScale
        AppSettings.floatingPetCustomScale = 0
    }

    override func tearDown() {
        AppSettings.floatingPetCustomScale = originalFloatingPetCustomScale
        super.tearDown()
    }
    ...
    func testWindowOriginPreservesPetLeadingEdgeAndCenterWhenPetScalesUpWithLeftBubble() {
        // 旧布局：宠物在窗口右侧，气泡在左侧并与宠物垂直居中
        let lastLayout = DetachedIslandWindowLayout(
            containerSize: CGSize(width: 540, height: 104),
            petFrame: CGRect(x: 400, y: 2, width: 100, height: 100),
            bubbleFrame: CGRect(x: 2, y: 2, width: 396, height: 100),
            bubblePlacement: .topLeft,
            petAnchorInWindow: CGPoint(x: 450, y: 52),
            bubbleContentMode: .hoverPreview
        )
        let currentFrame = NSRect(origin: CGPoint(x: 1000, y: 100), size: lastLayout.containerSize)

        // 新布局：宠物放大，气泡仍在左侧并保持垂直居中
        let newLayout = DetachedIslandWindowLayout(
            containerSize: CGSize(width: 652, height: 204),
            petFrame: CGRect(x: 450, y: 2, width: 200, height: 200),
            bubbleFrame: CGRect(x: 2, y: 52, width: 446, height: 100),
            bubblePlacement: .topLeft,
            petAnchorInWindow: CGPoint(x: 550, y: 102),
            bubbleContentMode: .hoverPreview
        )

        let newOrigin = DetachedIslandWindowController.windowOrigin(
            preservingPetLeadingEdgeAndCenter: currentFrame,
            lastLayout: lastLayout,
            newLayout: newLayout
        )
        let newFrame = NSRect(origin: newOrigin, size: newLayout.containerSize)

        // 宠物左侧边缘屏幕位置保持不变
        XCTAssertEqual(
            newFrame.minX + newLayout.petFrame.minX,
            currentFrame.minX + lastLayout.petFrame.minX,
            accuracy: 0.5
        )
        // 宠物垂直中心屏幕位置保持不变
        let oldPetCenterY = currentFrame.maxY - lastLayout.petAnchorInWindow.y
        let newPetCenterY = newFrame.maxY - newLayout.petAnchorInWindow.y
        XCTAssertEqual(newPetCenterY, oldPetCenterY, accuracy: 0.5)

        // 气泡仍然紧贴宠物左侧（间距保持 leftBubbleGap）
        let bubbleRight = newFrame.minX + newLayout.bubbleFrame!.maxX
        let petLeft = newFrame.minX + newLayout.petFrame.minX
        XCTAssertEqual(
            petLeft - bubbleRight,
        ...
        // 气泡垂直中心与宠物垂直中心对齐
        let bubbleCenterY = newFrame.minY + newLayout.bubbleFrame!.midY
        XCTAssertEqual(bubbleCenterY, newPetCenterY, accuracy: 0.5)
    }
